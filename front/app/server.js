...
app.get("/get-config/:id", async (req, res) => {
  try {
    ...
    const zipFileName = `pipeline-${order.versioningTool[0]}-${order.hostingType[0]}-${order.operatingSystem[0]}-${order.monitoringTool[0]}-${order.hostingJarTool[0]}.zip`
    // const zipFileName = "pipeline-mulesoft.zip"
    const zipFilePath = path.join(basePathCustomer, zipFileName);
  ...
  if (order.versioningTool && order.hostingType && !order.monitoringTool && !order.hostingJarTool) {    
    ...
    const zipFileName = `pipeline-${order.versioningTool[0]}-${order.hostingType[0]}-${order.operatingSystem[0]}.zip`
    // const zipFileName = "pipeline-mulesoft.zip"
    const zipFilePath = path.join(basePathCustomer, zipFileName);
    const output = fs.createWriteStream(zipFilePath);
    const archive = archiver("zip", { zlib: { level: 9 } });
    ...
    if (!fs.existsSync(basePathCustomer)) {
      fs.mkdirSync(basePathCustomer, { recursive: true }); 
