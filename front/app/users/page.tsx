...
const UsersPage = () => {
  ...
  useEffect(() => {
    axios.get("https://devopsmule.cweave.fr/api/users") 
      .then(response => {
  ...
  const handleDeleteUser = (user: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios.delete(`https://devopsmule.cweave.fr/api/users/${user._id}`)
          .then(() => {
...
return (
    <div className="flex min-h-screen bg-gray-100">
      {/* Sidebar */}
      <div className={`${isCollapsed ? "w-16" : "w-64"} bg-[linear-gradient(to_right,#1e3c5a,#2a5d78)]  text-white h-screen transition-all duration-300 flex flex-col justify-between`}>
        <div>
          ...
          <div className="p-4 space-y-6">
            ...
            <a href="/orders" className={`flex items-center space-x-4 ${
          isActiveO ? "text-teal-500" : "text-white"
        } hover:text-teal-400`}>
              <FaBoxOpen className="text-2xl" /> {!isCollapsed && <span>Commandes</span>}
            </a>
            <a href="/users" className={`flex items-center space-x-4 ${
  ...
  } hover:text-teal-400 focus:outline-none`}        onClick={handleProfileClick}>
            ...
            {currentUsers.map((user:any) => (
              <div key={user._id} className="bg-white p-4 rounded-lg shadow border flex justify-between items-center">
                <div>
                  ...
                  <p className="text-gray-600">✉️ <a href={`mailto:${user.workEmail}`} className="text-teal-500">{user.workEmail}</a></p>                
                </div>
                <button 
