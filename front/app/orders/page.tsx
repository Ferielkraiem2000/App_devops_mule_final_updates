...
//  const isActiveO = location.pathname === "/orders"; 
//  const isActiveU = location.pathname === "/users"; 
  const pathname = usePathname();
  const isActiveO = pathname === "/orders";
  const isActiveU = pathname === "/users";
  ...
  const getCustomerName = async (customerId: any) => {
    try {
      ...
      const response = await axios.get(`https://devopsmule.cweave.fr/api/customer/${customerId}`);
      if (response.status !== 200) {
        throw new Error(`Unexpected status code: ${response.status}`);
      }
      return response.data.customerName;
    } catch (error: any) {
      console.error("Error fetching customer name:", error.message || error);
      return "N/A";
    }
  };
  ...
  const getOrders = async () => {
    try {
      const response = await axios.get(
        "https://devopsmule.cweave.fr/api/orders"
      );
  ...
  //   try {
  //     const response = await axios.post(`http://176.187.76.100:4000/accept-order/${orderId}`);
  //     if (response.status === 200) {
  ...
  //   setLoadingOrderId(orderId);
  //   try {
  //     const downloadUrl = `http://176.187.76.100:4000/get-config/${orderId}`;
  ...
  const handleAcceptOrder = async (orderId: string) => {
    setLoadingOrderId(orderId);
    try {
      const response = await fetch(`https://devopsmule.cweave.fr/api/get-config/${orderId}`);
      if (response.status === 200) {
        const updatedOrders: any = orders.map((o:any) =>
            o._id === orderId ? { ...o, status: "acceptée" } : o
        ); 
  ...
  const handleCancelOrder = async (order: any) => {
        try {
            const response = await axios.put(
                `https://devopsmule.cweave.fr/api/cancel-order/${order._id}`
            );
            if (response.status === 200) {
                const updatedOrders: any = orders.map((o:any) =>
  ...
  function confirmLogout(handleLogout:any) {
    ...
    return (
      <div className="flex min-h-screen overflow-y-auto bg-gray-100 flex-col ">
...
>
<div >
    <div className="flex justify-between items-center px-4 py-2 border-b border-gray-700 overflow-y-auto ">
      {!isCollapsed }
      <button
    ...
    <div className="p-4 space-y-6">
      ...
      <a
          ...
          className={`flex items-center space-x-4 ${
            isActiveO ? "text-teal-500" : "text-white"
          } hover:text-teal-400`}   
        >
      ...
      {/* <div className="flex items-center">
        ...
        <a
          href="/users"
          className={`flex items-center space-x-4 ${
            isActiveU ? "text-teal-500" : "text-white"
          } hover:text-teal-400`}     >
             ...
             <table className="w-full border-collapse">
              <thead>
                ...
                <th className="p-3 border" style={{ color: "black" }}>
                  ...
                  <th className="p-3 border" style={{ color: 'black' }}>Système d'exploitation</th>
                  <th className="p-3 border" style={{ color: "black" }}>
                    Outil de monitoring
                  </th>
                  <th className="p-3 border" style={{ color: "black" }}>
...
>
  ...
  />
                      ...
                      <td className={`p-3 border ${order.hostingType === "CloudHub 2.0" ? "text-gray-300" : "text-gray-500"}`}>
                            {order.hostingType === "Cloudhub2.0" ? "Non requis pour CloudHub 2.0" : order.operatingSystem}
                      </td>
                      <td className="p-3 border text-gray-500">
